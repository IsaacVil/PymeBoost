    ...
    <DashboardLayout>
      <div className="space-y-8">
        <div>
          ...
          <p className="text-zinc-500 mt-1">Welcome back, {session.email}</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          {mockStats.map((stat) => (
            <Card key={stat.label}>
              ...
              <p className="text-3xl font-bold text-teal-500 mt-1">{stat.value}</p>
            </Card>
          ))}
        </div>
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-semibold text-zinc-900">Upcoming Milestones</h2>
          {mockMilestones.map((ms) => (
            <Card key={ms.id}>
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-medium text-zinc-900">{ms.title}</p>
                  <p className="text-sm text-zinc-500">{ms.project}</p>
                </div>
