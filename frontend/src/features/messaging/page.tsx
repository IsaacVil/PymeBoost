        ...
        <div className="flex gap-6 h-[calc(100vh-14rem)]">
          ...
          <aside className="w-72 space-y-2 overflow-y-auto">
            {mockConversations.map((c) => (
              <Card key={c.id} className="cursor-pointer hover:border-purple-500/50 transition-colors p-4">
                <div className="flex items-center justify-between">
                  <p className="font-medium text-slate-100">{c.advisor}</p>
          ...
          <div className="flex-1 flex flex-col bg-slate-900 border border-slate-800 rounded-lg">
            <div className="p-4 border-b border-slate-800">
              <p className="font-semibold text-slate-100">Ana García</p>
              <p className="text-xs text-slate-400">Process Optimization Advisor</p>
            </div>
