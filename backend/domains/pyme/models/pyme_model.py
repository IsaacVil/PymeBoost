"""ORM model for PB_Pymes (PYME profile).

Maps the real schema in database/scripts/creationScript.sql §2. Columns with DB
defaults (id, reputationScore, ratingCount, createdAt, updatedAt) are omitted on
insert and filled by Postgres.
"""
from sqlalchemy import Column, DateTime, Integer, Numeric, String, Text, text
from sqlalchemy.dialects.postgresql import UUID

from backend.shared.database.base import Base


class PymeModel(Base):
    __tablename__ = "PB_Pymes"

    id = Column("id", UUID(as_uuid=False), primary_key=True, server_default=text("gen_random_uuid()"))
    auth0_id = Column("auth0Id", String(64), nullable=False, unique=True)
    owner_name = Column("ownerName", String(100), nullable=False)
    business_email = Column("businessEmail", String(150), nullable=False, unique=True)
    phone = Column("phone", String(20), nullable=False, unique=True)
    cedula_juridica = Column("cedulaJuridica", String(20), nullable=False, unique=True)
    company_name = Column("companyName", String(100), nullable=False)
    company_size_id = Column("companySizeId", UUID(as_uuid=False), nullable=False)
    industry_id = Column("industryId", UUID(as_uuid=False), nullable=True)
    description = Column("description", Text, nullable=True)
    pyme_status_id = Column("pymeStatusId", UUID(as_uuid=False), nullable=False)
    reputation_score = Column("reputationScore", Numeric(12, 8), nullable=False, server_default=text("0"))
    rating_count = Column("ratingCount", Integer, nullable=False, server_default=text("0"))
    created_at = Column("createdAt", DateTime(timezone=True), server_default=text("now()"))
    updated_at = Column("updatedAt", DateTime(timezone=True), server_default=text("now()"))
